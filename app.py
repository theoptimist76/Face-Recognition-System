...
class MainUI(tk.Tk):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.frames = {}
        for F in (StartPage, AdminPage, DashboardPage, PageOne, PageTwo, PageThree, PageFour):
    ...
    def on_closing(self):
        ...
        if messagebox.askokcancel("Quit", "Are you sure?"):
            global names
            f =  open("Detected Names/nameslist.txt", "a+")
            for i in names:
                    f.write(i+" ")
            self.destroy()
...
class StartPage(tk.Frame):
        ...
        def __init__(self, parent, controller):
            ...
            button3 = tk.Button(self, text="   Admin Login  ", fg="#ffffff", bg="#263942",command=lambda: self.controller.show_frame("AdminPage"))
        ...
        def on_closing(self):
            if messagebox.askokcancel("Quit", "Are you sure?"):
                global names
                with open("Detected Names/nameslist.txt", "w") as f:
                    for i in names:
                        f.write(i + " ")
...
class PageOne(tk.Frame):
    ...
    def start_training(self):
        ...
        if self.user_name.get() == "None":
            ...
            return
        # elif self.user_name.get() == str(self.user_name):
        #     messagebox.showerror("Error", "Please input string value")
        #     return
...
class AdminPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.controller = controller
        tk.Label(self, text="Enter the name", fg="#263942", font='Helvetica 12 bold').grid(row=0, column=0, pady=10, padx=5)
        tk.Label(self, text="Enter the pass", fg="#263942", font='Helvetica 12 bold').grid(row=1, column=0, pady=10, padx=5)
        self.uname = tk.Entry(self, borderwidth=3, bg="lightgrey", font='Helvetica 11')
        self.upass = tk.Entry(self, borderwidth=3, bg="lightgrey", font='Helvetica 11')
        self.uname.grid(row=0, column=1, pady=10, padx=10)
        self.upass.grid(row=1, column=1, pady=10, padx=10)
        self.buttoncanc = tk.Button(self, text="Home", bg="#ffffff", fg="#263942", command=lambda: controller.show_frame("StartPage"))
        self.buttonext = tk.Button(self, text="Next", fg="#ffffff", bg="#263942", command=lambda: controller.show_frame("DashboardPage"))
        self.buttoncanc.grid(row=5, column=0, pady=10, ipadx=5, ipady=4)
        self.buttonext.grid(row=5, column=1, pady=10, ipadx=5, ipady=4)
    def login(self):
        global names
        if self.uname.get() == "None" & self.upass.get() == "None":
            messagebox.showerror("Error", "Name cannot be 'None'")
            return
        elif len(self.uname.get()) == 0 & self.upass.get() == 0:
            messagebox.showerror("Error", "Name cannot be empty!")
            return
        elif self.uname.get() != str(self.uname):
            messagebox.showerror("Error", "Please input string value")
            return
        elif len(self.uname.get()) == 0 & len(self.upass.get()) == 0:
            messagebox.showerror("Error", "Please fill the inputs")
            return
        elif self.uname.get() == "admin" & self.upass.get() == "admin":
...
class PageThree(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        self.homebutton = tk.Button(self, text="Home Page", command=lambda: self.controller.show_frame("StartPage"), bg="#ffffff", fg="#263942")
        ...
        self.homebutton.grid(row=2, column=0, ipadx=5, ipady=4, padx=10, pady=20)
...
app = MainUI()
app.iconphoto(False, tk.PhotoImage(file='./files/icon.ico'))
app.mainloop()
